...
Otsakkeessa ( header) on tietoa esimerkiksi tiedoston koosta `content-length`, palvelimesta `server`, tiedosto-tyypistä `content type`. `curl -I` on hyödyllinen jos tarvitaan tietoa tiedoston koosta ja muokkauspäivästä eikä haluta ladata koko tiedostoa. (Lähde: https://www.warp.dev/terminus/curl-show-headers)  
