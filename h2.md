...
30.8 22.30
APT (Advanced Package Tool) on paketinhallintatyökalu Linuxille. Halusin asentaa koneelle samalla kertaa kolme itselleni uutta komentoriviohjelmaa. Valitsin ohjelmiksi Ranger, Trash-cli ja Ncdu ohjelmat. Kaikki ohjelmat voidaan ladata samalla kertaa käyttämällä komentoa `sudo apt-get update && sudo apt-get upgrade -y` ja `sudo apt-get install ranger trash-cli ncdu`. Ohjelmien asennukset onnistuivat ongelmitta.  
...
Ranger on tiedostonhallintatyökalu, jonka avulla esim. nuolinäppäimiä käyttämällä voi liikkua hakemistossa.

Trash on roskakorityökalu. Sen avulla voi mm. laittaa tiedostoja roskakoriin `trash-put foo`, listata roskakorin tiedostot `trash-list`
