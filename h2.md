...
Hakemistoa selatessa välilyönti menee yhden sivun eteenpäin, `b` menee sivun taaksepäin ja `q` pääset takaisin terminaliin.  

## f) Rauta  

Tässä osioss listattiin testaamani koneen rauta komennolla `sudo lshw -short -sanitize`. Lshw komentoa ei löytynyt, joten se tuli asentaa ensin. Annoin komennon `sudo apt install lshw`  
